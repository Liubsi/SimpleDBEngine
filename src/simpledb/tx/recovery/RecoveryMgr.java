...
public class RecoveryMgr {
   ...
   private void doRecover() {
      ...
      while (iter.hasNext()) {
         ...
         if (txlist != null && finishedTxs.containsAll(txlist)) {
            return;
         }
         if (rec.op() == NQCKPT) {
         ...
         } else if (!finishedTxs.contains(rec.txNumber()) && (txlist == null || txlist.contains(rec.txNumber()))) {
            rec.undo(tx); 
            finishedTxs.add(rec.txNumber());
         }
      }
   ...
   public void checkpoint(List<Integer> activeTxNums) {
      NQCheckpointRecord.writeToLog(lm, activeTxNums);
  }
}
